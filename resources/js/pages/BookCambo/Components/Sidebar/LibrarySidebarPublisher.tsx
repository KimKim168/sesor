...
export default function LibrarySidebarPublisher({
    heading = 'All Lists',
    options,
    limit = 5,
}: {
    ...
    return (
        <div className="flex flex-col gap-2">
            {visibleOptions.map((item) => (
                <div key={item.value} className="flex cursor-pointer items-center gap-3 rounded-md px-2 py-1 hover:bg-muted">
            ...
            {options.length > limit && (
                <button
                    ...
                    className="group flex items-center gap-1 text-left text-sm font-semibold text-primary underline underline-offset-4 transition-all duration-300 hover:translate-x-1"
                >
                    See More{' '}
                    <ArrowRight
