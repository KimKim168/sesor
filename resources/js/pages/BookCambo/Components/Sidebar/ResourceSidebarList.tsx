            ...
            <Accordion type="single" collapsible className="w-full rounded-lg">
                {visibleOptions.map((item) => (
                    <AccordionItem value={item.value} className="h-auto border-b-0">
                        <div className="flex">
                            <div key={item.value} className="flex flex-1 cursor-pointer items-center gap-3 rounded-md px-2 hover:bg-muted">
                                <Checkbox id={item.value} />
                                <Label htmlFor={item.value} className="cursor-pointer leading-snug">
                                    {item.label}
                                </Label>
                            </div>
                            <AccordionTrigger chevron={false} className="w-auto cursor-pointer h-auto py-1">
                        ...
                        {item.subcategories && (
              <AccordionContent>
                <ul className="ml-4 flex flex-col gap-2 border-l pl-2.5">
                  {item.subcategories.map((sub) => (
                    <li key={sub.value} className="flex items-center gap-3 rounded px-2 py-1 hover:bg-muted">
                      <Checkbox id={sub.value} />
                      <Label htmlFor={sub.value} className="cursor-pointer leading-snug">
                        {sub.label}
                      </Label>
                    </li>
                  ))}
