...
export default function ResourceSidebarListCollection({
    heading = 'All Lists',
    options,
    limit = 5,
}: {
            ...
            <Accordion type="single" collapsible className="w-full rounded-lg">
                {visibleOptions.map((item) => (
                    <AccordionItem value={item.value} className="h-auto border-b-0">
                        <div className="flex">
                            <div key={item.value} className="flex flex-1 py-2 cursor-pointer items-center gap-3 rounded-md px-2 hover:bg-muted">
                                <Checkbox id={item.value} />
                                <Label htmlFor={item.value} className="cursor-pointer leading-snug h-auto ">
                                    {item.label}
                                </Label>
                            </div>
