...
const AuthorSearch = ({ debounceSearch = true, buttonClassName = '' }: AuthorSearchProps) => {
    ...
    const isOnAuthorsPage = currentPath === '/authors';

    // Only debounce if enabled
    const debouncedSearch = useCallback(
        debounce((searchTerm: string) => {
            if (!isOnAuthorsPage || !debounceSearch) return;
        ...
        [currentPath, isOnAuthorsPage, debounceSearch],
    );
    ...
    const handleSearch = () => {
        if (isOnAuthorsPage && debounceSearch) {
            debouncedSearch(search);
        } else {
            const queryParams = new URLSearchParams();
            if (search) queryParams.set('search', search);
            router.get(`/authors?${queryParams.toString()}`);
        }
    };
            ...
            >
                ...
                <Input
                    ...
                    onChange={(e) => {
                        ...
                        if (isOnAuthorsPage && debounceSearch) debouncedSearch(e.target.value);
                    }}
